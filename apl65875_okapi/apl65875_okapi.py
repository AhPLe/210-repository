                ...
                if bid and ask:
                    ...
                    if len(self.avg_win1_list) > 0 and len(self.avg_win2_list) > 0 and len(self.orders) == 0:
                        if self.avg_win1_list[-1] >= self.avg_win2_list[-1]:
                            ...
                            if self.holdings['CASH'] >= ask:
                                buy_size = min(self.size, math.floor(self.holdings['CASH']/ask))
                                self.placeLimitOrder(self.symbol, quantity=buy_size, is_buy_order=True, limit_price=ask)
