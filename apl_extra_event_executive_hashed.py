...
class MYEventExecutive():
    ...
    def insert_LP(self, LP: MYSim_LP) -> int: #inserts a MYSim_LP into the list, then gives back the id
        value_hash = hash(LP)
            ...
            if not self.queue.empty():
                ...
                if msg[0] >= self.sim_Time:
                    ...
                    if not msg[2] in self.LP_dict: # >=len(self.LP_list)
                        ...
                        self.LP_dict[msg[2]].MYSimGetMsg(msg[0], msg[1], msg[3])
    ...
    def send_Next(self, time: int, transmit_id: int, message: Event_Enum, rand_recip: bool = False):
        ...
        if time > self.sim_Time or len(self.LP_dict) - 1 < 1:
            ...
            tempLP_list = list(self.LP_dict.keys()) #this is awful programming
            transmit_index = tempLP_list.index(transmit_id)
            if not rand_recip:
                recipient_id = tempLP_list[(transmit_index+1)%len(self.LP_dict)]
            else:
                temp_rand = random.randint(1, len(self.LP_dict) - 1) #random without sending back to same node
                if temp_rand < transmit_index:
                    recipient_id = tempLP_list[temp_rand]
                else:
                    recipient_id = tempLP_list[(temp_rand+1)%len(self.LP_dict)] 
            if self.VERBOSE:
