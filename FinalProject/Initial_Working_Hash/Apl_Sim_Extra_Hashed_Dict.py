...
import math
import apl_extra_event_executive_hashed as ev_exec
    ...
    def Initialize(LPList_Initial):
        ...
        for i in range(new_infected):
            try: 
                ...
                #end of LPList, there's nothing else to infect
                #if i+1<new_infected:
                #    fut[1].InitializeInfection()
                #    print('fut is', fut)
                #if i+2<new_infected:
                #    print('warning, newly infected percent over number of potential agents') 
                break #nothing else to infect, file should stop
            
            print(item)
            #item_test = next(item_iter)
            
            #cur, fut = item_iter
            #print('cur is', cur)
            item[1].InitializeInfection()
            
            #cur[1].InitializeInfection()
            
            
        #for item in item_iter:
            #LPList_Initial[i][1].InitializeInfection()
#            if count < new_infected: #this is actually the end of the loop, but it allows item_iter to iterate easily
# #                item[1].InitializeInfection() #intialize the infection of the item
...
for i in range(len(spread) - 1):
    ...
    ex.Reset_Random() #resets seed so it starts over from the first runtime
...
ax.set_xlabel('Simulation Ending at Time')
